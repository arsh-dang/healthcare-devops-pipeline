...
node {
    ...
    try {
        ...
        sendSlackNotification("""🚨 Pipeline Initialization Failed - ${params.BUILD_TYPE} build for ${params.ENVIRONMENT}
...
**Next Steps:**
Please check the pipeline logs and fix the initialization error.""", 'danger')
        
        // Send email notification if configured
        if (params.SEND_EMAIL && params.EMAIL_RECIPIENTS) {
            sendEmailNotification(
                "Pipeline Initialization Failed - Healthcare App",
                """<h2>Pipeline Initialization Failed</h2>
                    ...
                    try {
                        withCredentials([
                            ...
                            sh '''
                                ...
                                # Deploy Datadog infrastructure using Terraform
                                echo "Deploying Datadog infrastructure..."
                                cd datadog/scripts
                                ...
                                if [ -n "$DATADOG_API_KEY" ]; then
                                    curl -s -H "DD-API-KEY: $DATADOG_API_KEY" "https://api.us5.datadoghq.com/api/v1/validate" | jq . || echo "API key validation failed"
                                else
                                ...
                                if ./deploy-datadog.sh --dry-run > /tmp/datadog_plan.log 2>&1; then
                                    ...
                                    echo "Datadog infrastructure plan failed"
                                    cat /tmp/datadog_plan.log
                                    exit 1
                                fi
                            '''
                        }
                                ...
                                sh '''
                                    ...
                                    if command -v npm >/dev/null 2>&1; then
                                        ...
                                        if [ -f "server/package.json" ]; then
                                            cd server
                                            npm install --legacy-peer-deps --force || echo "Backend dependencies installed"
                                            cd ..
                                        else
                                            npm install --legacy-peer-deps --force || echo "Backend dependencies installed"
                                        fi
                                    ...
                                    if command -v docker >/dev/null 2>&1; then
                                        ...
                                        if [ "$FRONTEND_BUILT" = true ]; then
                                            ...
                                            if ! docker images nginx:1.25.3-alpine | grep -q "1.25.3-alpine"; then
                                                ...
                                                # Build frontend Docker image with robust dependency handling
                                        ...
                                        if [ "$BACKEND_BUILT" = true ]; then
                                            ...
                                            if ! docker images node:20-alpine | grep -q "20-alpine"; then
                                                ...
                                                # Build backend Docker image with robust dependency handling
                                ...
                                sh '''
                                    ...
                                    if command -v npm >/dev/null 2>&1; then
                                        ...
                                        if [ -f "pnpm-lock.yaml" ]; then
                                            ...
                                            npm ci --cache .npm --legacy-peer-deps --force >/dev/null 2>&1 || echo "Dependencies already installed"
                                        else
                                            pnpm install --no-frozen-lockfile >/dev/null 2>&1 || echo "Dependencies already installed"
                                        fi
...
EOF
                                    ...
                                    if command -v npm >/dev/null 2>&1; then
                                        ...
                                        npm install --legacy-peer-deps --force >/dev/null 2>&1 || echo "Dependencies already installed"
...
EOF
                                ...
                                cat <<EOF | curl -X POST "https://api.us5.datadoghq.com/api/v1/events" \\
                                    -H "Content-Type: application/json" \\
                                    -H "DD-API-KEY: \$DATADOG_API_KEY" \\
                                    -d @-
{
...
EOF
                                    ...
                                    SECURE_CONFIGS=$(timeout 30 find . -name "*.tf" -print0 | xargs -0 grep -l "security_group" 2>/dev/null | wc -l || echo "0")
                                    SECURE_CONFIGS=$((SECURE_CONFIGS + $(timeout 30 find . -name "*.tf" -print0 | xargs -0 grep -l "firewall" 2>/dev/null | wc -l || echo "0")))
                                    SECURE_CONFIGS=$((SECURE_CONFIGS + $(timeout 30 find . -name "*.tf" -print0 | xargs -0 grep -l "encryption" 2>/dev/null | wc -l || echo "0")))
                                    PUBLIC_IPS=$(timeout 30 find . -name "*.tf" -print0 | xargs -0 grep -l "associate_public_ip_address.*true" 2>/dev/null | wc -l || echo "0")
...
EOF
                                ...
                                timeout 180 TF_VAR_smtp_username="$SMTP_USER" TF_VAR_smtp_password="$SMTP_PASS" TF_VAR_smtp_from_email="$SMTP_USER" TF_VAR_alert_email_critical="$SMTP_RECIPIENT" TF_VAR_alert_email_warning="$SMTP_RECIPIENT" TF_VAR_alert_email_info="$SMTP_RECIPIENT" TF_VAR_datadog_api_key="$DATADOG_API_KEY" terraform apply -auto-approve tfplan || {
...
EOF
                                ...
                                timeout 300 TF_VAR_smtp_username="$SMTP_USER" TF_VAR_smtp_password="$SMTP_PASS" TF_VAR_smtp_from_email="$SMTP_USER" TF_VAR_alert_email_critical="$SMTP_RECIPIENT" TF_VAR_alert_email_warning="$SMTP_RECIPIENT" TF_VAR_alert_email_info="$SMTP_RECIPIENT" TF_VAR_datadog_api_key="$DATADOG_API_KEY" terraform apply -auto-approve tfplan-green || {
                    ...
                    -d @-
{
    ...
    "text": "Healthcare App CI/CD Pipeline failed. Check Jenkins logs for details.",
    "priority": "high",
