...
if terraform workspace select $ENVIRONMENT 2>/dev/null; then
    echo "[SUCCESS] Selected existing workspace: $ENVIRONMENT"
else
    echo "Creating new workspace: $ENVIRONMENT"
    if terraform workspace new $ENVIRONMENT; then
        echo "[SUCCESS] Workspace '$ENVIRONMENT' created successfully"
    else
        echo "[WARNING] Workspace creation might have failed, but continuing..."
        # Try to select again in case it was created by another process
        terraform workspace select $ENVIRONMENT || {
            echo "[ERROR] Failed to create or select workspace: $ENVIRONMENT"
            exit 1
        }
    fi
fi

echo "[SUCCESS] Terraform workspace '$ENVIRONMENT' is ready"
