    ...
    template {
      ...
      spec {
        ...
        volume {
          name = "portainer-data"
          persistent_volume_claim {
            claim_name = kubernetes_persistent_volume_claim.portainer_storage.metadata[0].name
          }
        }
...
resource "kubernetes_persistent_volume_claim" "portainer_storage" {
  metadata {
...
resource "kubernetes_service" "sonarqube" {
  ...
  spec {
    ...
    port {
      name        = "sonarqube"
      port        = 9001  # External port 9001
      target_port = 9000  # Internal SonarQube port
      protocol    = "TCP"
    }
...
# Output URLs for new services
output "portainer_url" {
  description = "Portainer URL for internal access"
  value       = "http://${kubernetes_service.portainer.metadata[0].name}.${kubernetes_namespace.monitoring.metadata[0].name}.svc.cluster.local:9000"
}

output "sonarqube_url" {
  description = "SonarQube URL for internal access"
  value       = "http://${kubernetes_service.sonarqube.metadata[0].name}.${kubernetes_namespace.monitoring.metadata[0].name}.svc.cluster.local:9001"
}
